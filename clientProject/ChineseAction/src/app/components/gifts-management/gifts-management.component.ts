...
@Component({
  ...
  templateUrl: './gifts-management.component.html',
  styleUrl: './gifts-management.component.css'
})
export class GiftsManagementComponent {
  giftSrv: giftsService = inject(giftsService);

  giftsList$ = this.giftSrv.getAll();
  //primeNg
...
//   // }
  ...
  add(title: string, description: string, donorId: number, categoryId: number, price: number, numOfBuyers: number, image: string | undefined) {
    if (title && description && donorId && categoryId && price && numOfBuyers && image)
      this.giftSrv.add({ id: 0, title: title, description: description, donorId: donorId, categoryId: categoryId, price: price, numOfBuyers: numOfBuyers, image: image }).subscribe(date => {
        this.giftsList$ = this.giftSrv.getAll();
      });
  }

  save(item: gift) {
    this.giftSrv.update(item).subscribe(data => { });
  }
